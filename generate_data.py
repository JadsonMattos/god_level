...
def setup_base_data(conn):
    ...
    # Insert brand first
    print(f"Inserting brand with id {BRAND_ID}...")
    cursor.execute(
        "INSERT INTO brands (id, name) VALUES (%s, %s) ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name",
        (BRAND_ID, "Challenge Restaurants")
    )
    print(f"Brand insert executed. Committing...")
    conn.commit()  # Commit the brand insertion
    print(f"Brand committed. Verifying...")
    
    # Verify brand exists
    cursor.execute("SELECT id FROM brands WHERE id = %s", (BRAND_ID,))
    result = cursor.fetchone()
    if not result:
        raise Exception(f"Failed to insert brand with id {BRAND_ID}")
    print(f"Brand {BRAND_ID} verified successfully!")
